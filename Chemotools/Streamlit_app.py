...
def get_barsplot(k) :
        ...
        # BOX PLOT
        file_synt = (mol for name in unique_name for mol in file_in if mol.GetProp(st.session_state.molecule_name) == name)
        index2 = []
        ...
        data_frame2 = pd.DataFrame(array2, index=index2, columns=[st.session_state.score])
...
#
        ...
        data_frame2.sort_values(st.session_state.score, ascending=False, inplace=True)
        sns.set_style('whitegrid')
        sns.set_context('talk')
        g = sns.catplot(x=st.session_state.score, y='Name', data=data_frame2, kind = "box", palette="rocket", aspect=st.session_state.aspect_plot, height=st.session_state.height_plot)
        ...
        sns.set_context('talk')
        pal = sns.cubehelix_palette(len(data_frame2.index), rot=-.25, light=.7)
        g = sns.FacetGrid(data_frame2, row="Name", hue="Name", aspect=st.session_state.aspect_density_plot, height=st.session_state.height_density_plot, palette="rocket")
        g.map(sns.kdeplot, st.session_state.score,
              bw_adjust=.5, clip_on=False,
              fill=True, alpha=1, linewidth=1.5)
        g.map(sns.kdeplot, st.session_state.score, clip_on=False, color="w", lw=2, bw_adjust=.5)
        ...
        if 'fig2' not in st.session_state:
            ...
            if st.session_state.numbers_conformation != 1 :
                ...
                st.session_state.help_paragraph = ("To give an idea, if your number of molecules (not number of poses) = 15 :\n"
                                 ...
                                 " Height density plot = 0.3, Gap = -0.55\n "
                                 "\nif your number of molecules (not number of poses) = 75 :\n - Aspect ratio = 1.75,"
                                 " Height = 18, Xlabels Size = 25, Ylabels Size = 15, Aspect ratio density plot = 30,"
                                 " Height density plot = 0.5, Gap = -0.35"

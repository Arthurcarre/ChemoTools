...
def presets(self_cmd, sele):
    r = [[2, 'Preset:', '']]
    with menucontext(self_cmd, sele) as mc:
        if 'r_watermap_color_r' in mc.props:
            r += [
                [1, 'WaterMap (precomputed)', 'preset.watermap("' + sele + '","none",_self=cmd)'],
            ]
        if 'r_watermap_free_energy' in mc.props or 'r_watermap_potential_energy' in mc.props:
            r += [
                [1, 'WaterMap dG (simple) absolute', 'preset.watermap("' + sele + '","green red",_self=cmd)'],
                [1, 'WaterMap dG (simple) relative', 'preset.watermap("' + sele + '","green red",absolute=False,_self=cmd)'],
                [1, 'WaterMap dG (red/blue) absolute', 'preset.watermap("' + sele + '","blue red",_self=cmd)'],
                [1, 'WaterMap dG (red/blue) relative', 'preset.watermap("' + sele + '","blue red",absolute=False,_self=cmd)'],
                [1, 'WaterMap dH and -TdS', 'preset.watermap("' + sele + '","complex",_self=cmd)'],
                [0, '', ''],
            ]
            if mc.oname is not None and self_cmd.get_type(mc.oname) == 'object:molecule':
                r += [
                    [1, 'WaterMap sphere mode', [
                        [ 2, 'Sphere Mode:' ],
                        [ 1, 'spheres',     f'cmd.set("sphere_mode",-1,{mc.oname!r},quiet=0)' ],
                        [ 1, 'cubes',       f'cmd.set("sphere_mode",10,{mc.oname!r},quiet=0)' ],
                        [ 1, 'tetrahedra',  f'cmd.set("sphere_mode",11,{mc.oname!r},quiet=0)' ],
                    ]],
                    [0],
                ]
    r += [
...
def find(self_cmd, sele):
    return [[ 2, 'Find:', ''],
              ...
              [ 1, 'clashes', [[ 2, 'Clashes:', '']] + [
                  [1, 'vdw-distance ratio < 0.89', 'cmd.distance("'+sele+'_clashes089","'+sele+'","same",reset=1,mode=8,cutoff=0.89)'],
                  [1, 'vdw-distance ratio < 0.75', 'cmd.distance("'+sele+'_clashes075","'+sele+'","same",reset=1,mode=8,cutoff=0.75)'],
              ]],
              ]
...
def mol_labels(self_cmd, sele):
    with menucontext(self_cmd, sele) as mc:
        ...
        if 'r_watermap_potential_energy' in mc.props:
            r.append([1, 'WaterMap', [[ 2, 'WaterMap:', '' ],
                [1, "dH",   'cmd.label("' + sele +
                    '", "\'\' if p.r_watermap_potential_energy is None else (p.r_watermap_potential_energy + 19.67)")'],
                [1, "-TdS", 'cmd.label("' + sele +
                    '", "\'\' if p.r_watermap_entropy is None else p.r_watermap_entropy")'],
                [1, "dG",   'cmd.label("' + sele +
                    '", "\'\' if p.r_watermap_potential_energy is None else (p.r_watermap_potential_energy + p.r_watermap_entropy + 19.67)")'],
            ]])
        return r

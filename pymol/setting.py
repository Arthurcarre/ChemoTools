...
'''
        ...
        with _self.lockcm:
            return _cmd.set(_self._COb, index, v, selection,
                         int(state) - 1, int(quiet), int(updates))

    @undoSessionDecorator("Unset")
...
SEE ALSO
    ...
    unset_deep, set, set_bond
        ...
        with _self.lockcm:
            return _cmd.unset(_self._COb, index, selection,
                           int(state) - 1, int(quiet), int(updates))
...
PYMOL API
    ...
    @undoSessionDecorator("Unset Deep")

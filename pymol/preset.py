...
from . import util
import traceback
from .undo import undoSessionDecorator
...
@undoSessionDecorator("Preset: Simple")
...
@undoSessionDecorator("Preset: Simple No solvent")
...
@undoSessionDecorator("Preset: Ligands")
...
@undoSessionDecorator("Preset: Ball and Stick")
...
@undoSessionDecorator("Preset: B-factor Putty")
...
@undoSessionDecorator("Preset: Ligand Cartoon")
def ligand_cartoon(selection="(all)",*,_self=cmd):
    cmd=_self
    s, selection = ligand_sites(selection, _self=_self)[:2]
...
@undoSessionDecorator("Preset: Ligand Sites")
...
@undoSessionDecorator("Preset: Ligand Sites HQ")
...
@undoSessionDecorator("Preset: Ligand Sites Trans")
...
@undoSessionDecorator("Preset: Ligand Sites Trans HQ")
...
@undoSessionDecorator("Preset: Ligand Sites Mesh")
...
@undoSessionDecorator("Preset: Ligand Sites Dots")
...
@undoSessionDecorator("Preset: Technical")
...
@undoSessionDecorator("Preset: Default")
...
@undoSessionDecorator("Preset: Interface")
...
@undoSessionDecorator("Preset: Classified")
...
@undoSessionDecorator("Preset: Watermap")
def watermap(selection='*', palette='', absolute=True,*,_self=cmd):
    '''
DESCRIPTION

    WaterMap colored spheres preset.

    Requires atom properties:
    - palette "none":
      - r_watermap_color_r
      - r_watermap_color_g
      - r_watermap_color_b
    - palette "complex":
      - r_watermap_potential_energy
      - r_watermap_entropy
    - color palettes:
      - r_watermap_potential_energy
      - r_watermap_entropy
      - (or r_watermap_free_energy)

ARGUMENTS

    selection = str: atom selection {default: all}

    palette = str: spectrum palette for dG coloring, or "complex" for dH and
    -TdS coloring, "none" for using stored r_watermap_color_r/g/b properties.
    {default: first try "none", then dG "green red"}

    absolute = 0/1: use absolute color scale {default: 1}
    '''
    try:
        from pymol import watermapping as wm
    except ImportError:
        import pymol
        raise pymol.IncentiveOnlyException(
            'WaterMap presets only available in Incentive PyMOL')

    absolute = int(absolute)

    if not palette:
        if wm.watermap_predefined(selection, quiet=1, _self=_self):
            return True
        palette = "green red"

    if palette == "none":
        assert absolute
        return wm.watermap_predefined(selection, _self=_self)

    if palette == "complex":
        assert absolute
        return wm.watermap_complex(selection, _self=_self)

    return wm.watermap_simple(selection, palette, absolute, _self=_self)

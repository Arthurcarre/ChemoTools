...
#####################################################
##                                                 ##
## -- CIT CONFORMATIONTOOL UNIQUE CONFORMATION --  ##
##                                                 ##
#####################################################
"""
        ...
        Proportion = st.slider('Minimum size of the sample defining a predominant binding mode. Default proportion = 0.05',
                             0.0, 1.0, 0.15,
                               help=('This setting define the minimum proportion (value between 0 and 1) of individuals'
                                     ...
                                     ' representative of a full predominant binding mode.'), key = 'CIT_WEB_Unique_Molecule_ConformationTool')
            ...
            if third_checkbox :
                ...
                if 'sorted_heatmap' not in st.session_state:
                    ...
                    with open("Best_PLPScore_Poses.sdf", "rb") as file:
                         btn = st.download_button(
                                    label="Download the SDF file including each of the representatives from a predominant binding mode",
                                    data=file,
                    ...
                    st.write("RMSD value between each representative from each predominant binding mode.")
                    ...
                    st.write("Density of the number of poses as a function of the RMSD calculated between the representative from each predominant binding mode"
                ...
                if st.session_state.n_conformations != 1:
                    ...
                    st.session_state.temp = st.select_slider("You want a sdf file and/or a anlysis plots including molecules in the predominant binding mode n°",
                    ...
                    st.info('There is only one predominant predominant binding mode. Do you want to have the sdf file of poses in'
                            ' this predominant binding mode ? ')
                    st.session_state.RMSD_threshold_conformation = st.slider(
                        'You want a sdf file and/or a analysis plots including molecules in the unique predominant binding mode'
                    ...
                    if "cluster_hierarchy_heatmap" in st.session_state :
                        ...
                        if st.button('Get analysis', key = 'CIT_WEB_Unique_Molecule_ConformationTool'):
                            ...
                            if "n_conformations" in st.session_state and st.session_state.n_clusters_selected == st.session_state.n_clusters:
                                ...
                                st.write("Density of the number of poses as a function of the RMSD calculated between the representative of"
                                         " each predominant binding mode and all poses of all molecules in the docking solutions of the"
                                         " filtered incoming sdf file.")      
                                
                                temp_options = range(1, st.session_state.n_conformations + 1)
                                if "temp" not in st.session_state :
                                    st.session_state.temp = 1
                                st.session_state.temp = st.select_slider("You want a sdf file and/or a analysis plots including molecules"
                                                                         " in the predominant binding mode n°",
                                                                         options=temp_options,
                                                                         value = st.session_state.temp,
                                                                         key = 'CIT_WEB_Unique_Molecule_ConformationTool')
                                ...
                                if st.button('Prepare your sdf file', key = 'CIT_WEB_Unique_Molecule_ConformationTool'):
                                    ...
                                    if os.path.exists(f'Predominant Binding Mode n°{st.session_state.temp}.sdf') == True :
                                        with open(f'Predominant Binding Mode n°{st.session_state.temp}.sdf', "rb") as file:
                                             btn = st.download_button(
                                                        label="Download your sdf file",
                                                        data=file,

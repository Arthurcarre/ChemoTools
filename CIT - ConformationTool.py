...
class ConformationTool :
    ...
    def get_plots(self,
        ...
        ### SECOND BOX PLOT, SCATTER PLOT AND DENSITY PLOT
        ...
        sns.set_context('talk')
        g = sns.lmplot(x='Median', y='Ratio', data = df5, palette = 'purple', aspect=aspect_plot,
                        height=height_plot, order = 2)
        label_point(df5.Median, df5.Ratio, df5.Name, plt.gca())
        g.set_axis_labels(self.score, "")
        g.set_yticklabels(fontsize = size_ylabels + 10)
        g.set_xticklabels(fontsize = size_xlabels)
        plt.show()

...
def get_barsplot(k) :
        ...
        unique_name = get_unique_numbers(list(table.index))        
        sns.set_context('talk')
        g = sns.catplot(x="Ratio", y=list(table.index), data = table, kind="bar", aspect=1.75, height=18, palette = "rocket")
        g.set_ylabels("")
        xlocs, xlabs = plt.xticks()
        for i, v in enumerate(list(table["Ratio"])):
            plt.text(float(v)+0.03, float(i)+0.35, f"{round(v*100, 1)}%", horizontalalignment = "center", fontsize=20)
        plt.yticks(fontsize=18)
        plt.axvline(x=0.5, color="black", linestyle="--")
        plt.axvline(x=0.25, color="black", linestyle=":")
        plt.axvline(x=0.75, color="black", linestyle=":")
        #plt.annotate("50%", (0.47, i+0.4), fontsize=25)
        #plt.annotate("25%", (0.22, i+0.4), fontsize=25)
        #plt.annotate("75%", (0.72, i+0.4), fontsize=25)
        g.set_axis_labels("Ratio", "", fontsize = 20)
        ...
        # BOX PLOT
        unique_name = get_unique_numbers(list(table.index))
        file_synt = (mol for name in unique_name for mol in file_in if mol.GetProp("SourceTag") == name)
        index2 = []
        columns2 = "Docking Score"
        array2 = np.zeros(shape=(len(file_in),1))
        for i, mol in enumerate(file_synt):
                    index2.append(mol.GetProp("SourceTag"))
                    array2[i, 0] = mol.GetProp(st.session_state.score)
        data_frame2 = pd.DataFrame(array2, index=index2, columns=["Docking Score"])
        data_frame2.reset_index(inplace=True)
        data_frame2.rename(columns={'index': 'Name'}, inplace=True)
        sns.set_context('talk')
        g = sns.catplot(x='Docking Score', y='Name', data=data_frame2, kind = "box", palette="rocket", aspect=1.75, height=18)
        g.set_axis_labels("ChemPLP Score", "", fontsize = 20)
        g.set_yticklabels(fontsize = 20)
        g.set_xticklabels(fontsize = 30)
        ...
        g = sns.FacetGrid(data_frame2, row="Name", hue="Name", aspect=30, height=.4, palette="rocket")

        # Draw the densities in a few steps
        g.map(sns.kdeplot, "Docking Score",
              bw_adjust=.5, clip_on=False,
              fill=True, alpha=1, linewidth=1.5)
        g.map(sns.kdeplot, "Docking Score", clip_on=False, color="w", lw=2, bw_adjust=.5)
...
if first_checkbox :
    ...
    try :
        st.session_state.mols[0].GetProp("Gold.PLP.Fitness")
        st.session_state.score = "Gold.PLP.Fitness"
    except :
        st.session_state.score = "Gold.Goldscore.Fitness"
        ...
        try:
            ...
            if st.session_state.numbers_conformation != 1 :
                ...
                if st.button('Prepare your sdf file and build plots'):             
                    ...
                    with st.spinner('Please wait, plots are coming...'):              
                        ...
                        with open(f"Box_Plot{st.session_state.temp}.jpeg", "rb") as file:
                             btn = st.download_button(
                                     ...
                                     data=file,
                                     file_name=f"Boxplot_Conformation n°{st.session_state.temp}.jpeg",
                                     mime="image/jpeg")
                        st.pyplot(st.session_state.fig6)
                        with open(f"Density_Plot{st.session_state.temp}.jpeg", "rb") as file:
                             btn = st.download_button(
                                     label=f"Download Density PLOT Conformation n°{st.session_state.temp} ",
                                     data=file,
                                     file_name=f"Densityplot_Conformation n°{st.session_state.temp}.jpeg",
                                     mime="image/jpeg")
                    ...
                    with st.spinner('Please wait, the barplot is coming...'):
                        ...
                        with open(f"Box_Plot1.jpeg", "rb") as file:
                             btn = st.download_button(
                                     ...
                                     data=file,
                                     file_name=f"Boxplot_Conformation n°1.jpeg",
                                     mime="image/jpeg")
                        st.pyplot(st.session_state.fig6)
                        with open(f"Density_Plot1.jpeg", "rb") as file:
                             btn = st.download_button(
                                     label=f"Download Density PLOT Conformation n°1 ",
                                     data=file,
                                     file_name=f"Densityplot_Conformation n°1.jpeg",
                                     mime="image/jpeg")
                else :
                    try :
